...
from evidently.metrics import (ColumnDriftMetric, ColumnQuantileMetric,
                               DataQualityStabilityMetric, DatasetDriftMetric,
...
reference_data = pd.read_parquet("data/march_reference.parquet")
...
def calculate_metrics_postgressql(curr, i):
    ...
    fare_income_quantile = result["metrics"][3]["result"]["current"]["value"]
    curr.execute(
        "insert into dummy_metrics(timestamp, prediction_drift, num_drifted_columns, share_missing_val, fare_income_quantile) values (%s, %s, %s, %s, %s)",
        (
            begin + datetime.timedelta(i),
            prediction_drift,
            num_drifted_columns,
            share_missing_val,
            fare_income_quantile,
        ),
    )
